...
forceChart = function () {
	var width = 400,
		...
		xStart = function (d) {
			return x(d) + 50 * Math.random() - 25;
		},
		yStart = function (d) {
			return y(d) + 50 * Math.random() - 25;
		},
		rStart = function (d) {
			return r(d);
		},
	...
	var force = layout.force().charge(0).gravity(0);

	function chart(selection, nodes) {
		if (shape === "circle") {
			collide = collideCircle;
		} else if (shape === "square") {
			collide = collideSquare;
		} else {
			console.error("forceChart.shape must be 'circle' or 'square'");
		}
		...
		force.size([width, height]).nodes(nodes).on("tick", tick).start();
